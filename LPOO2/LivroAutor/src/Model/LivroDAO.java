...
public class LivroDAO {
    ...
    public List<Autor> listarAutores(int idLivro) throws Exception {
        ...
        try {
            con = ConnectionFactory.getConnection();
            st = con.prepareStatement("SELECT autor.id, autor.nome FROM autor INNER JOIN livro_autor ON autor.id = livro_autor.idAutor WHERE livro_autor.idLivro = ?");
            st.setInt(1, idLivro);

            rs = st.executeQuery();
            List<Autor> autores = new ArrayList<Autor>();
            while (rs.next()) {
                int id = rs.getInt("autor.id");
                String nome = rs.getString("autor.nome");
