...
int main(int argc, char *argv[]) {
	...
	//retira os espaços e pontuações da string e armazena em um novo vetor
	...
	//pegar o tamanho do vetor formatado para poder fazer o array de tras pra frente	
	for(size = 0; size < 30; size++){
		if(phraseNoSpaces[size] == '\0'){
			break;
		}
	}
	
	//armazena a string formatada de tras pra frente em um novo vetor
	for(i = 0, j = size - 1; i < size, j >= 0; i++, j--){
		if(phraseNoSpaces[i] == '\0'){
			break;
		}
